...
function Home() {
  ...
  const [listDataTop10Movies, setListDataTop10Movies] =
    React.useState<Movies[]>();
  const [listDataTop10ComingTheaters, setListDataTop10ComingTheaters] =
    React.useState<Movies[]>();
  ...
  const getComingSoonMovies = () => {
     ...
     */
    const newArray = listDataTop10ComingTheatersTemp.sort(
      (a, b) => b.likes - a.likes
    );
    return newArray;
  };
  ...
  useEffect(() => {
    if (windowSize.current[0] < 1450) {
      console.log(
        "🚀 ~ file: Home.tsx:60 ~ useEffect ~ windowSize.current[0]:",
        windowSize.current[0]
      );
      console.log(
        "🚀 ~ file: Home.tsx:58 ~ useEffect ~ windowSize.current[1]:",
  ...
  return (
    <div className="flex justify-start items-center flex-col container mt-4 mb-4">
      <div className="flex w-full justify-start flex-row  items-center">
        ...
        <div id="newsLetter" className="w-1/4">
          <NewsLetter />
        </div>
      </div>
      <div className="flex flex-col w-full">
        <div className="flex items-center w-full justify-start pt-8 pb-3 ml-6">
          <LabelTitle
            customClass="font-bold ml-1"
            title="MOVIES RELEASED TODAY"
          />
        </div>
        {carrouselList ? (
          <>
            {
              <Carousel show={carrouselItems} withIndicator>

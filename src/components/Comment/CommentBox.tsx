...
function CommentBox({onSaveCommentEvent, setRaiting, raiting,}: CommentBoxInterface) {
        ...
        <div className="w-2/4 mb-4">
          ...
          <div className="flex w-full justify-between">
                <div className="w-full">
                  <label htmlFor="first_name" className="block mb-2 font-bold text- text-sm   dark:text-white">Full Name</label>
                  <input 
                    type="text" 
                    id="first_name"
                    onChange={inputHandlerName} value={title} 
          ...
          <label htmlFor="comment" className="block mb-2 text-sm  font-bold  dark:text-white">What did you think of the movie? (optional)</label>
          <textarea id="message"
              value={comment}
              onChange={inputHandler} rows={4}
              className="block p-2.5 mb-2 w-full text-sm  text-black rounded-lg border border-gray-300
               focus:ring-blue-500 focus:border-blue-500  dark:placeholder-gray-400  dark:focus:ring-blue-500 dark:focus:border-blue-500"></textarea>
          </div>
          <div className="flex items-center justify-between px-0 py-2 ">
            <Button label={postLabel} type="submit" customClass="disabled:opacity-75 inline-flex items-center py-2.5 px-0 text-xs font-medium text-center bg-yellow-500"/>
          </div>
        </div>
      </form>
    </div>
